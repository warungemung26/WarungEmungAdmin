...
document.getElementById('uploadTableJsonBtn').addEventListener('click', () => {
  const log = document.getElementById('offlineLog');

  try {
    // Ambil data dari tabel HTML, misal tabel id="produkTable"
    const table = document.getElementById('produkTable');
    const data = [];
    
    for (let i = 1; i < table.rows.length; i++) { // mulai dari 1 untuk skip header
    ...
    saveProdukOffline(data); // pakai fungsi yang sudah ada
